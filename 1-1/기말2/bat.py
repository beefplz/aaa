...
class Bat:
    ...
    def detect_collision(self, ball, sides_sweet_spot=True,
        ...
        if(collision):
            ...
            elif((left_b>left-r)and(rigt_b<right+r)and
                 (top_b<top)and(bottom_b>=top)):
                collision_direction="N"
                ball.y_speed=-abs(ball.y_speed)
            else:
                collision_direction="miss"
            if((sides_sweet_spot==True)and(collision_direction=="E"
                                           or collision_direction=="W")):
                adjustment=(-(v_centre-v_centre_b))/(self.height/2)
                ball.y_speed=feel*adjustment
            if((topnbottom_sweet_spot==True)and(collision_direction=="N" or
                                                collision_direction=="S")):
