...
class Bat:
    ...
    def detect_collision(self, ball, sides_sweet_spot=True, topnbottom_sweet_spot=False):
        ...
        v_centre_b=top_b+r
        h_centre_b=left_b+r
        if((bottom_b > top) and (top_b<bottom)and(right_b>right)and(left_b<right)):
            collision=True
        if(collision):
            if((top_b > top-r)and(bottom_b<bottom+r)and(right_b>right)and(left_b<=right)):
                ...
                ball.y_speed=-abs(ball.x_speed)
            else:
                collision_direction="miss"
            if((sides_sweet_spot==True)and(collision_direction=="W"or collision_direction=="S")):
                adjustment = (-(v_centre - v_centre_b))/(self.width/2)
