...
class Table:
    def __init__(self, window, color="black", width=600, height=400,
        ...
        self.canvas.pack()
        if(vertical_net):
            self.canvas.create_line(selfwidth/s, 0, self.width/2, self.height,
                                    width=2, fill=self.net_color, dash=(15, 23))
        if(horizontal_net):
            self.canvas.create_line(0, self.height/2, self.width, self.height,
