...
# 레이블 생성
Label(window, text="정의되어 있는 단어를 입려하고 엔터 키를 누르세요:").grid(row=0, column=0, sticky=W)

import random

questions = ("알고리즘", "인수", "2진수", "버그", "형변환", "주석")

#2진수의 숫자2가 에러를 일으킴

def click():
    choice = random.randint(0, 6)
    entered_text = questions[choice]
    output.delete(0.0, END)
    definition = my_glossary[entered_text]
    output.insert(END, definition)

Button(window, text="무작위", width=5, command=click).grid(row=1, column=0, sticky=W)
...
Button(window, text="제출", width=5, command=click).grid(row=3, column=0, sticky=W)



# 다른 레이블 생성
Label(window, text="\n정의:").grid(row=4, column=0, sticky=W)
